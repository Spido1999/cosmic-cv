...
class ResumeGenerator:
    ...
    def generate(self, user_profile: str, parsed_jd: dict[str, Any],
        ...
        resume_data = self._generate_initial(user_profile, parsed_jd, allow_title_change)
    ...
    def _generate_initial(self, user_profile: str, jd: dict, allow_title_change: bool = False) -> dict:
...
Now use that analysis to generate the ideal resume.
...
MUST-HAVE KEYWORDS (embed ALL in summary + bullets):
...
KEY RESPONSIBILITIES (map EVERY experience bullet to one of these):
...
3. Each experience bullet maps to a specific responsibility above
4. Use the EXACT keyword/phrase the company uses
5. Projects MUST fill any JD skill gaps not covered by experience
6. Every bullet needs a quantified result at {jd.get('seniority_level', '')} scale
7. Skills categories must mirror this JD's domain focus
8. The resume must make a recruiter think: "This person has done exactly what we need"

Now generate the complete ATS-optimised resume JSON:"""
